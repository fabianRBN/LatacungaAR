...
import { AtractivoService, ArchivoService, AuthService } from '../../../services/service.index';
...
export class CrearAtractivoComponent implements OnInit {
  ...
  constructor(public atractivoService: AtractivoService,
  ...
  guardarImagenes(){
    ...
    this.imgTemporales.forEach((imgTem , index)=>{
      ...
      if (imgTem.archivo != null) {
        const ubicacion = 'imagenes/atractivos/' + this.atractivo.id+'-'+index+''+this.date.getTime()+ this.date.getMilliseconds();
        ...
        //   });
        var sp = null;
        this.archivoService.subirArchivo(imgTem.archivo, ubicacion)
          .on(firebase.storage.TaskEvent.STATE_CHANGED,
            (snapshot)=>{
              ...
              if(progreso >= 100){
                ...
                this.imagenes.titulo = this.imgTemporales[index].titulo;
                this.atractivoService.cargarImagenes(this.atractivo.id, this.imagenes)
              }
